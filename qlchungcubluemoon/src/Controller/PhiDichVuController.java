...
public class PhiDichVuController {
    ...
    public void changeFeeOnAction(ActionEvent event) {
        ...
        if(newFee <= 0){
            ControllerUtil2.showErrorMessage("Phí dịch vụ mới không hợp lệ, vui lòng nhập lại!");
            return;
        }
        boolean confirmed = ControllerUtil2.showConfirmationDialog("Xác nhận thay đổi phí dịch vụ chung cư" , "Bạn có chắc chắn muốn thay đổi phí dịch vụ chung cư không ?");
        if(confirmed){
            MysqlConnector.getInstance().changeFeeData(tenPhi, newFee, Year.now().getValue());
            ControllerUtil2.showSuccessAlert("Thay đổi phí dịch vụ chung cư thành công!");
            feeList = MysqlConnector.getInstance().getFeeData(tenPhi, yearCBox.getValue());
            feeTableView.setItems(feeList);
            feeTableView.refresh();
